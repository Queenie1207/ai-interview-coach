...
describe("interview analyze route", () => {
  ...
  it.each([[new AiNotConfiguredError(), "AI_NOT_CONFIGURED", 503], [new AnalysisEmptyOutputError(), "AI_EMPTY_OUTPUT", 502], [new AnalysisInvalidJsonError(), "AI_INVALID_JSON", 502], [new AnalysisInvalidOutputError(), "AI_INVALID_OUTPUT", 502], [new AnalysisUpstreamError("secret prompt and API key"), "AI_UPSTREAM_ERROR", 502]] as const)("maps safe failure %s", async (error, code, status) => { mocked.mockRejectedValueOnce(error); const response = await POST(req(JSON.stringify({ resume: validResume, jobDescription: jd }))); expect(response.status).toBe(status); const body = await response.json(); expect(body.error.code).toBe(code); expect(JSON.stringify(body)).not.toContain("secret prompt"); });
  it("returns success/data", async () => { mocked.mockResolvedValueOnce(validAnalysis); const response = await POST(req(JSON.stringify({ resume: validResume, jobDescription: jd }))); expect(response.status).toBe(200); expect(await response.json()).toEqual({ success: true, data: validAnalysis }); });
});
